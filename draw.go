...
// draw: defines and implements all methods needed
...
// Map containing all valid colors
...
func colorUnknown(c Color) bool {
	var colorsRev = map[Color]string{
		{255, 0, 0}:     "red",
		{0, 255, 0}:     "green",
		{0, 0, 255}:     "blue",
		{255, 255, 0}:   "yellow",
...
func (rect Rectangle) draw(scn screen) (err error) {
	// check for spatial/color errors
	ul := Point{rect.ll.x, rect.ur.y}
	lr := Point{rect.ur.x, rect.ll.y}
	if outOfBounds(ul, scn) || outOfBounds(rect.ur, scn) || outOfBounds(rect.ll, scn) || outOfBounds(lr, scn) {
...
func (tri Triangle) printShape() string {
	line := fmt.Sprintf("Triangle: (%v,%v), (%v,%v), (%v,%v)", tri.pt0.x, tri.pt0.y, tri.pt1.x, tri.pt1.y, tri.pt2.x, tri.pt2.y)
	return line
}
...
func (circ Circle) draw(scn screen) (err error) {
	...
	up := Point{circ.center.x, circ.center.y + circ.r}
	down := Point{circ.center.x, circ.center.y - circ.r}
	left := Point{circ.center.x - circ.r, circ.center.y}
	right := Point{circ.center.x + circ.r, circ.center.y}
	if outOfBounds(circ.center, scn) || outOfBounds(up, scn) || outOfBounds(down, scn) || outOfBounds(left, scn) || outOfBounds(right, scn) {
		return outOfBoundsErr
	}
...
func (circ Circle) printShape() string {
	line := fmt.Sprintf("Circle: centered around (%v,%v) with radius %v", circ.center.x, circ.center.y, circ.r)
	return line
}
...
func (display *Display) clearScreen() {
	for i := 0; i < display.maxY; i++ {
		for j := 0; j < display.maxX; j++ {
			display.matrix[j][i] = Color{255, 255, 255}
		}
	}
}
...
func (display *Display) drawPixel(x, y int, c Color) (err error) {
	if x >= display.maxX || x < 0 || y >= display.maxY || y < 0 {
		return outOfBoundsErr
	}
	if colorUnknown(c) {
		return colorUnknownErr
...
func (display *Display) getPixel(x, y int) (c Color, err error) {
	color := display.matrix[x][y]
	if colorUnknown(color) {
		return color, colorUnknownErr
	}
	return color, nil
}
...
func (display *Display) screenShot(f string) (err error) {
	...
	for i := 0; i < display.maxX; i++ {
		for j := 0; j < display.maxY; j++ {
			pixel := display.matrix[i][j]
			fmt.Fprint(file, pixel.r, pixel.g, pixel.b, " ")
		}
		fmt.Fprintln(file, "")
	}
	return
